...
print("✅ 技术指标计算完成（RSI 24天）")

# 显示数据前几行（包含指标）
print("\n数据预览（前30行）:")
print(data[['日期', '收盘', 'MA250', 'RSI']].head(30))

# 回测设置
initial_capital = 100000  # 初始资金10万
print("\n" + "="*60)
print(f"开始回测（RSI 24天）- 初始资金: {initial_capital:,} 元")
print("="*60)
...
# 保存结果
portfolio_df.to_csv("回测结果_组合市值_RSI24.csv", index=False, encoding='utf-8-sig')
if len(trade_df) &gt; 0:
    trade_df.to_csv("回测结果_交易记录_RSI24.csv", index=False, encoding='utf-8-sig')
yearly_df.to_csv("回测结果_年度收益率_RSI24.csv", index=False, encoding='utf-8-sig')

print("\n✅ 回测结果已保存！")
print("- 回测结果_组合市值_RSI24.csv")
print("- 回测结果_交易记录_RSI24.csv")
print("- 回测结果_年度收益率_RSI24.csv")
